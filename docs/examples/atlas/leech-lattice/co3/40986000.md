# $U_3(3):2$ as a subgroup of Conway group Co~3~
...
$$
    H=\langle b,c,f,g\rangle \cong U_3(3):2,
$$

then

$$
    [\mathrm{Co}_3:H]=40,986,000.
$$
...
The recorded run took about 12 minutes and 3 seconds.
...
if actual_index != 40_986_000:
    raise RuntimeError(f"expected index 40986000, got {actual_index}")
```
...
??? info "Truncated output from the Python script"
    ...
    #0: ToddCoxeter: time spent in phases |   2min37s (22%) |             - (0%) |            - (0%) | 9min25s (78%)
    #0: ToddCoxeter: phase 0.2 = 2min38s  | run 0 = 12min3s | all runs = 12min3s | elapsed = 12min3s
    The index of the subgroup is 40986000
    ```
