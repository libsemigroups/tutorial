# $S_4$ as a subgroup of the McLaughlin group McL
...
$$
    H=\langle c,d,e\rangle \cong S_4,
$$

then

$$
    [\mathrm{McL}:H]=37,422,000.
$$
...
The recorded run took about 6 minutes 47 seconds.
...
??? info "Truncated output from the Python script"
    ...
    #0: ToddCoxeter: time spent in phases |   1min39s (24%) |             - (0%) |            - (0%) | 5min8s (76%)
    #0: ToddCoxeter: phase 0.2 = 1min39s  | run 0 = 6min47s | all runs = 6min47s | elapsed = 6min47s
    The index of the subgroup is 37422000
    ```
