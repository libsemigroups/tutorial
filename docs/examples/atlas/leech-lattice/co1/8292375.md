# $2^{11}:\mathrm{M}_{24}$ as a subgroup of Conway group Co~1~
...
$$
    H=\langle a,b,d,e,f,g,h,(gfdc)^4,z\rangle
      \cong 2^{12}.\mathrm{M}_{24},
$$

then $H/\langle z\rangle\cong 2^{11}:\mathrm{M}_{24}$ and

$$
    [\mathrm{Co}_1:H/\langle z\rangle]=[G:H]=8,292,375.
$$
...
Todd-Coxeter algorithm. The recorded run took about 1 minute and 57 seconds.
...
for word in (*"abdefgh", "gfdc" * 4, centre):
    tc.add_generating_pair(word, "")

tc.run()
actual_index = tc.number_of_classes()
print(f"The index of the subgroup is {actual_index}")
if actual_index != 8_292_375:
    raise RuntimeError(f"expected index 8292375, got {actual_index}")
```
...
??? info "Truncated output from the Python script"
    ...
    #0: ToddCoxeter: time spent in phases |   24.962s (21%) |             - (0%) |            - (0%) | 1min32s (79%)
    #0: ToddCoxeter: phase 0.2 = 25.173s  | run 0 = 1min57s | all runs = 1min57s | elapsed = 1min57s
    The index of the subgroup is 8292375
    ```
